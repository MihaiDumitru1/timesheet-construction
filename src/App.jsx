    ...
    if (filters.length > 0) {
      const body = {
        structuredQuery: {
          ...
          orderBy: filters[0]?.orderBy ? [{ field: { fieldPath: filters[0].orderBy }, direction: filters[0].orderDir || "DESCENDING" }] : undefined,
        }
      };
...
:root {
  --ink: #0d1117; --ink2: #161b22; --ink3: #21262d; --ink4: #30363d;
  --muted: #6e7681; --text: #c9d1d9; --bright: #f0f6fc;
  --amber: #f0883e; --amber-dim: #d97234; --amber-glow: rgba(240,136,62,.15);
  --green: #3fb950; --red: #f85149; --blue: #58a6ff;
  --r: 8px; --sw: 228px;
}
html,body { height:100%; background:var(--ink); color:var(--text); font-family:'Sora',sans-serif; }

/* Layout */
.layout { display:flex; height:100vh; overflow:hidden; }
.sidebar { width:var(--sw); background:var(--ink2); border-right:1px solid var(--ink4); display:flex; flex-direction:column; flex-shrink:0; }
.main { flex:1; display:flex; flex-direction:column; overflow:hidden; }
.topbar { height:56px; background:var(--ink2); border-bottom:1px solid var(--ink4); display:flex; align-items:center; justify-content:space-between; padding:0 28px; flex-shrink:0; }
.page { flex:1; overflow-y:auto; padding:28px; }

/* Sidebar */
.sb-logo { padding:20px 18px 14px; border-bottom:1px solid var(--ink4); }
.sb-logo h1 { font-size:15px; font-weight:800; color:var(--bright); letter-spacing:.5px; }
.sb-logo p { font-family:'DM Mono'; font-size:10px; color:var(--amber); letter-spacing:2px; text-transform:uppercase; margin-top:3px; }
.sb-sec { padding:14px 14px 4px; font-family:'DM Mono'; font-size:10px; color:var(--muted); letter-spacing:1.5px; text-transform:uppercase; }
.sb-nav { padding:4px 10px; display:flex; flex-direction:column; gap:2px; flex:1; }
.sb-btn { display:flex; align-items:center; gap:10px; padding:8px 10px; border-radius:6px; font-size:13px; font-weight:500; color:var(--text); cursor:pointer; border:none; background:none; width:100%; text-align:left; transition:all .12s; }
.sb-btn:hover { background:var(--ink3); color:var(--bright); }
.sb-btn.active { background:var(--amber-glow); color:var(--amber); font-weight:700; }
.sb-foot { border-top:1px solid var(--ink4); padding:14px; }
.sb-user { display:flex; align-items:center; gap:10px; }
.ava { width:32px; height:32px; border-radius:50%; background:var(--amber); color:var(--ink); font-weight:800; font-size:12px; display:flex; align-items:center; justify-content:center; flex-shrink:0; font-family:'DM Mono'; }
.ava-sm { width:28px; height:28px; font-size:11px; }
.u-name { font-size:13px; font-weight:600; color:var(--bright); white-space:nowrap; overflow:hidden; text-overflow:ellipsis; }
.u-role { font-family:'DM Mono'; font-size:10px; color:var(--muted); text-transform:uppercase; letter-spacing:1px; }

/* Buttons */
.btn { display:inline-flex; align-items:center; gap:7px; padding:7px 16px; border-radius:var(--r); font-size:13px; font-weight:600; cursor:pointer; border:none; font-family:'Sora'; transition:all .12s; white-space:nowrap; }
.btn-p { background:var(--amber); color:var(--ink); }
.btn-p:hover { background:var(--amber-dim); }
.btn-s { background:var(--ink3); color:var(--text); border:1px solid var(--ink4); }
.btn-s:hover { background:var(--ink4); color:var(--bright); }
.btn-d { background:rgba(248,81,73,.1); color:var(--red); border:1px solid rgba(248,81,73,.2); }
.btn-d:hover { background:rgba(248,81,73,.2); }
...
/* Cards */
.card { background:var(--ink2); border:1px solid var(--ink4); border-radius:var(--r); }
.card-h { padding:14px 20px; border-bottom:1px solid var(--ink4); display:flex; align-items:center; justify-content:space-between; }
...
.stat { background:var(--ink2); border:1px solid var(--ink4); border-radius:var(--r); padding:18px 20px; position:relative; overflow:hidden; }
.stat::after { content:''; position:absolute; top:0; left:0; right:0; height:2px; background:var(--amber); }
.stat-l { font-family:'DM Mono'; font-size:10px; color:var(--muted); text-transform:uppercase; letter-spacing:1.5px; margin-bottom:8px; }
.stat-v { font-size:30px; font-weight:800; color:var(--bright); line-height:1; }
.stat-s { font-size:12px; color:var(--muted); margin-top:4px; }

/* Table */
.tw { overflow-x:auto; }
table { width:100%; border-collapse:collapse; }
th { padding:10px 16px; text-align:left; font-family:'DM Mono'; font-size:10px; color:var(--muted); text-transform:uppercase; letter-spacing:1.5px; border-bottom:1px solid var(--ink4); white-space:nowrap; }
td { padding:12px 16px; font-size:13px; border-bottom:1px solid rgba(255,255,255,.04); vertical-align:middle; }
tr:last-child td { border-bottom:none; }
tr:hover td { background:rgba(255,255,255,.02); }

/* Badges */
.bdg { display:inline-flex; align-items:center; padding:2px 9px; border-radius:99px; font-family:'DM Mono'; font-size:10px; font-weight:500; letter-spacing:.5px; text-transform:uppercase; }
.bdg-a { background:var(--amber-glow); color:var(--amber); border:1px solid rgba(240,136,62,.2); }
.bdg-g { background:rgba(63,185,80,.1); color:var(--green); border:1px solid rgba(63,185,80,.2); }
.bdg-b { background:rgba(88,166,255,.1); color:var(--blue); border:1px solid rgba(88,166,255,.2); }
.bdg-m { background:var(--ink3); color:var(--muted); border:1px solid var(--ink4); }

/* Forms */
.fld { display:flex; flex-direction:column; gap:6px; }
.fld label { font-family:'DM Mono'; font-size:10px; color:var(--muted); text-transform:uppercase; letter-spacing:1.2px; }
.fld input,.fld select,.fld textarea { background:var(--ink3); border:1px solid var(--ink4); border-radius:6px; padding:9px 13px; font-size:13px; color:var(--bright); font-family:'Sora'; outline:none; transition:border-color .12s; width:100%; }
.fld input:focus,.fld select:focus,.fld textarea:focus { border-color:var(--amber); }
.fld select option { background:var(--ink2); }
...
.modal { background:var(--ink2); border:1px solid var(--ink4); border-radius:12px; width:100%; max-width:480px; box-shadow:0 24px 64px rgba(0,0,0,.5); }
.modal-h { padding:20px 24px 0; display:flex; align-items:center; justify-content:space-between; }
.modal-h h3 { font-size:16px; font-weight:700; color:var(--bright); }
.modal-b { padding:20px 24px; display:flex; flex-direction:column; gap:14px; }
.modal-f { padding:0 24px 20px; display:flex; gap:10px; justify-content:flex-end; }
...
.chip { background:var(--amber-glow); border:1px solid rgba(240,136,62,.2); border-radius:99px; padding:3px 10px; font-size:11px; color:var(--amber); font-weight:600; font-family:'DM Mono'; }
.err { background:rgba(248,81,73,.1); border:1px solid rgba(248,81,73,.2); border-radius:6px; padding:10px 14px; font-size:12px; color:var(--red); }
.ok  { background:rgba(63,185,80,.1); border:1px solid rgba(63,185,80,.2); border-radius:6px; padding:10px 14px; font-size:12px; color:var(--green); }
.divider { height:1px; background:var(--ink4); margin:8px 0; }

/* Login */
.login-wrap { min-height:100vh; display:flex; align-items:center; justify-content:center; background:var(--ink); }
.login-card { background:var(--ink2); border:1px solid var(--ink4); border-radius:14px; padding:44px 40px; width:100%; max-width:400px; box-shadow:0 32px 80px rgba(0,0,0,.5); position:relative; overflow:hidden; }
.login-card::before { content:''; position:absolute; top:0; left:0; right:0; height:3px; background:linear-gradient(90deg,var(--amber),#ff6b35); }
.login-logo { margin-bottom:32px; }
.login-logo h1 { font-size:24px; font-weight:800; color:var(--bright); }
.login-logo p { font-family:'DM Mono'; font-size:10px; color:var(--amber); text-transform:uppercase; letter-spacing:2px; margin-top:4px; }

/* Spinner */
.spin { display:inline-block; width:14px; height:14px; border:2px solid rgba(255,255,255,.2); border-top-color:var(--amber); border-radius:50%; animation:spin .6s linear infinite; }
          ...
          {recent.length === 0
            ...
            : <div style={{ padding: "8px 16px", display: "flex", flexDirection: "column", gap: 6 }}>
              {recent.map(e => {
                ...
                return (
                  <div key={e.id} style={{ display: "flex", alignItems: "center", gap: 12, padding: "8px 0", borderBottom: "1px solid var(--ink4)" }}>
      ...
      <div className="modal">
        ...
        <div className="modal-b">
          {eligible.length === 0
            ...
            : eligible.map(u => (
              <div key={u.id} style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "10px 14px", background: "var(--ink3)", borderRadius: 6 }}>
...
function MyTimesheets() {
  ...
  useEffect(() => {
    Promise.all([
      API.getDocs("timesheets", [{ field: "userId", value: profile.id, orderBy: "workDate", orderDir: "DESCENDING" }]),
